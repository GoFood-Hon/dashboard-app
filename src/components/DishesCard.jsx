...
export default function DishesCard({ dish }) {
  const { status = "Habilitado", name, images, price, isUserSearch, isUserLove, isPromotion } = dish
  ...
  return (
    <div className="w-full px-6 py-3 bg-white rounded-2xl border border-blue-100 flex-col justify-end items-center inline-flex">
      ...
      <Image h={140} w="auto" fit="contain" src={images[0].location} alt={name} />
      <div className="flex flex-row justify-between w-full">
        <div className="flex-col items-start gap-2 flex w-full">
          <div className="text-right text-sky-950 text-base font-bold">{name}</div>
